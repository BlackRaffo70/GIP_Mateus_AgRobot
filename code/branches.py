...
def visualize_with_cutting_plan(segments, points, cutting_decisions, prune_decisions):
    ...
    Colors:
     - Tree trunk: cyan
     - Entirely cut branches: ORANGE (all removed)
     - Pruned branches: removed parts ORANGE, kept parts GREEN
     - Other branches default pink
    Axis frame is shown at cloud center and camera is aligned so Z is vertical (standing view).
    """
    colors = np.full((len(points), 3), [0.2, 0.2, 0.2])  # dark background
    # Color by decision
    for seg_id, seg in segments.items():
        class_name = [c['title'] for c in metadata['classes'] if c['id'] == seg['class_id']][0]
        if class_name == 'Tree':
            colors[seg['indices']] = [0.0, 0.76, 1.0]  # cyan
        elif seg_id in cutting_decisions:
            colors[seg['indices']] = [1.0, 0.5, 0.0]   # orange = removed
        elif seg_id in prune_decisions:
            remove_idx = np.array(prune_decisions[seg_id]['indices_to_remove'], dtype=int)
            keep_idx = np.array(prune_decisions[seg_id]['indices_to_keep'], dtype=int)
            if remove_idx.size:
                colors[remove_idx] = [1.0, 0.5, 0.0]  # orange = removed
            if keep_idx.size:
                colors[keep_idx] = [0.0, 1.0, 0.0]    # green = kept (gems)
        else:
            colors[seg['indices']] = [1.0, 0.0, 0.47]  # default branch pink

    pcd_vis = o3d.geometry.PointCloud()
    pcd_vis.points = o3d.utility.Vector3dVector(points)
    pcd_vis.colors = o3d.utility.Vector3dVector(colors)

    # coordinate frame at cloud center
    center = points.mean(axis=0)
    axis_length = max(0.2, np.linalg.norm(points.max(axis=0)-points.min(axis=0)) * 0.2)
    axis = o3d.geometry.TriangleMesh.create_coordinate_frame(size=axis_length, origin=center)

    # Create visualizer and set camera to "standing" view: Z up, looking along -Y
    vis = o3d.visualization.Visualizer()
    vis.create_window(window_name='Cutting Plan', width=1280, height=720)
    vis.add_geometry(pcd_vis)
    vis.add_geometry(axis)

    ctr = vis.get_view_control()
    # define camera orientation: front vector (direction camera looks toward), up vector, lookat point
    front = np.array([0.0, -1.0, 0.0])     # look horizontally along -Y
    up = np.array([0.0, 0.0, 1.0])         # Z axis as up (standing)
    # normalize front/up and ensure orthogonality
    ...
    # zoom: lower -> more zoomed in; tune as needed
    ctr.set_zoom(0.7)
